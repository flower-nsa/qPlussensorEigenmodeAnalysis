% --- 基本パラメータの設定 ---
% 探針長さを固定 (3 mm)
startl_point_end_l = 0.5e-3:0.1e-3:3.5e-3;%探針の長さパラメータ
...
% 結果を保存
results = [results; double([param.l*1e3, f_fre_1, k1_eff_v, k1_eff_l,angle1, f_fre_2, k2_eff_v, k2_eff_l,angle2,...
    f_fre_3, k3_eff_v, k3_eff_l,angle3])];  % 単位:mm, Hz

end

% テーブル表示
T = array2table(results, 'VariableNames', {'l_mm', 'f1_Hz', 'k1_eff_v', 'k1_eff_l','angle1','f2_Hz', 'k2_eff_v', 'k2_eff_l','angle2',...
    'f3_Hz','k3_eff_v', 'k3_eff_l','angle3',});
disp(T)
...
%共振1
function [k_eff_v, k_eff_l,angle] = compute_k_theta(X1,X2,X3,X4,X5,f,param)
omegak = 2 * pi * f;
